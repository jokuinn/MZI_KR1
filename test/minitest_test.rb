...
class MinitestTest < Minitest::Test
  include Gost3411Algorithm
  def test_that_it_has_a_version_number
    refute_nil ::Gost3411Algorithm::VERSION
  end

  def test_M1_32
    digest = @gost.new(32).update(M1).final
    assert digest == Et1_32
  end

  def test_M1_64
    digest = Gost3411.new(64).update(M1).final
    assert digest == Et1_64
  end

  def test_M2_32
    digest = Gost3411.new(32).update(M2).final
    assert digest == Et2_32
  end

  def test_M2_64
    digest = Gost3411.new(64).update(M2).final
    assert digest == Et2_64
  end

  def test_M64_32
    digest = Gost3411.new(32).update(M64).final
    assert digest == Et64_32
  end

  def test_M64_64
    digest = Gost3411.new(64).update(M64).final
    assert digest == Et64_64
  end

  def test_empty_32
    digest = Gost3411.new(32).final
    assert digest == Et0_32
  end

  def test_empty_64
    digest = Gost3411.new(64).final
    assert digest == Et0_64
  end

  def test_size_not_set
    digest = Gost3411.new.final
    assert digest == Et0_64
  end

  def test_not_32_nor_64
    digest = Gost3411.new(20).final
    assert digest == Et0_64
  end
end
